...
var fullPersonaeByArcana = (function () {
    ...
    for (var i = 0; i < fullPersonaeList.length; i++) {
        var persona = fullPersonaeList[i];
...
function checkSkill() {
    var isGood = true;
    for (var i = 0; i < fullPersonaeList.length; i++) {
        for (var key in fullPersonaeList[i].skills) {
            if (fullPersonaeList[i].skills.hasOwnProperty(key)) {
                if (!skillMap[key]) {
                    isGood = false;
